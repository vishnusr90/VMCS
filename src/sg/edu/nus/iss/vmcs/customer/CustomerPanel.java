...
public class CustomerPanel extends Dialog  implements MoneyReceiverObserver {
    ...
    public void  MoneyReceiverStateChanged(MoneyReceiver source, MoneyReceiverState newState){
        switch(newState){
            ...
            case InvalidMoney:
                    ...
                    if(source instanceof CoinReceiver) this.setChange("Invalid Coin!");
                    if(source instanceof NoteReceiver) this.setChange("Invalid Note!");
                    break;
            case  Refunded:
    ...
    private MoneyInputBox coinInputBox;
    ...
    public CustomerPanel(Frame fr, TransactionController ctrl) {
        ...
        MoneyStore coinStore = (MoneyStore) txCtrl.getMainController().getStoreController().getStore(Store.COIN);
        MoneyStore noteStore = (MoneyStore) txCtrl.getMainController().getStoreController().getStore(Store.NOTE);
        List<MoneyStore> stores;
        stores = new ArrayList<>();
        stores.add(coinStore);
        stores.add(noteStore);
        coinInputBox = new MoneyInputBox(txCtrl.getMoneyReceivers());
     ...
     */
    public MoneyInputBox getCoinInputBox(){
            return coinInputBox;
    }
